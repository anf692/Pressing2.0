...
export const Route = createFileRoute("/_authenticated/articles")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Gestion de la grille tarifaire du pressing." },
    ],
  }),
...
function PageArticles() {
  ...
  const enregistrer = async () => {
    ...
    if (enEdition) {
      const { error } = await supabase
        .from("articles")
        .update({ nom: nom.trim(), prix: prixNum, type_prix: typePrix })
        .eq("id", enEdition.id);
      if (error) {
      ...
      const { error } = await supabase
        .from("articles")
        .insert({ nom: nom.trim(), prix: prixNum, type_prix: typePrix });
      if (error) {
        toast.error("Erreur : " + error.message);
        return;
      }
  ...
  const supprimer = async (id: string) => {
    ...
    if (error) {
      toast.error("Impossible de supprimer (article peut-être utilisé) : " + error.message);
      return;
    }
              ...
              {(articles ?? []).map((a) => (
                <TableRow key={a.id}>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="icon" onClick={() => ouvrirEdition(a)}>
