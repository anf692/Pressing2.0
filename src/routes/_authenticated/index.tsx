...
export const Route = createFileRoute("/_authenticated/")({
  head: () => ({
    meta: [
      { title: "Tableau de bord — Pressing by Ramou Diouf" },
      { name: "description", content: "Indicateurs clés et statistiques du pressing." },
    ],
  }),
...
function TableauDeBord() {
  ...
  const kpis = useMemo(() => {
    ...
    const caJour = cs
      .filter((c) => c.date_depot >= debutJ)
      .reduce((s, c) => s + (c.total_fcfa ?? 0), 0);
    const caMois = cs
      .filter((c) => c.date_depot >= debutM)
      .reduce((s, c) => s + (c.total_fcfa ?? 0), 0);
    const nbJour = cs.filter((c) => c.date_depot >= debutJ).length;
    const nbTotal = cs.length;
    const enAttenteRecup = cs.filter(
      (c) => c.statut === "en_attente" || c.statut === "en_cours" || c.statut === "pret",
    ).length;
    const recupMois = cs.filter(
      (c) =>
        c.statut === "recupere" &&
        c.date_recuperation_reelle &&
        c.date_recuperation_reelle >= debutM,
    ).length;
    ...
    <div className="container mx-auto space-y-4 p-4 sm:space-y-6 sm:p-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <CarteKpi
          ...
          valeur={formaterFCFA(kpis.caJour)}
          icone={Coins}
        />
        <CarteKpi
          titre="Chiffre d'affaires du mois"
          valeur={formaterFCFA(kpis.caMois)}
          icone={TrendingUp}
        />
        ...
        <CarteKpi
          titre="En attente de récupération"
          valeur={kpis.enAttenteRecup}
          icone={Clock}
        />
        <CarteKpi
          titre="Récupérées ce mois"
          valeur={kpis.recupMois}
          icone={CheckCircle2}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Top 5 articles commandés</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
