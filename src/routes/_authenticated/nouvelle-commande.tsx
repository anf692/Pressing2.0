...
export const Route = createFileRoute("/_authenticated/nouvelle-commande")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Créer une nouvelle commande de pressing." },
    ],
  }),
  component: PageNouvelleCommande,
});

function PageNouvelleCommande() {
  ...
  const supprimerLigne = (cle: string) => {
    setLignes((l) => l.filter((x) => x.cleLigne !== cle));
  };
    ...
    setLignes((l) =>
      l.map((x) => {
        ...
        if (modif.article_id) {
          ...
          if (a?.type_prix === "kilo") {
            fusion.quantite = null;
            fusion.poids_kg = fusion.poids_kg ?? 1;
          } else {
            fusion.poids_kg = null;
            fusion.quantite = fusion.quantite ?? 1;
          }
        }
        return fusion;
      }),
  ...
  const total = useMemo(
    () => lignes.reduce((s, l) => s + sousTotal(l), 0),
  ...
  const enregistrer = async () => {
    ...
    for (const l of lignes) {
      if (!l.article_id) {
        toast.error("Sélectionnez un article pour chaque ligne");
        return;
      }
      const a = articles?.find((x) => x.id === l.article_id);
      if (a?.type_prix === "kilo" && (!l.poids_kg || l.poids_kg <= 0)) {
        toast.error(`Poids invalide pour ${a.nom}`);
        return;
      }
      if (a?.type_prix === "fixe" && (!l.quantite || l.quantite <= 0)) {
        toast.error(`Quantité invalide pour ${a?.nom}`);
        return;
      }
    }
    ...
    try {
      ...
      const { data: commande, error: errCmd } = await supabase
        ...
        .insert({
          client_id: client.id,
          total_fcfa: total,
          date_recuperation_estimee: dateRecup || null,
        })
        .select()
        .single();
        ...
        <CardContent className="space-y-3">
          ...
          {lignes.map((l) => {
            ...
            return (
              <div
                key={l.cleLigne}
                className="grid grid-cols-1 items-end gap-3 rounded-lg border bg-muted/30 p-3 md:grid-cols-[2fr_1fr_1fr_auto]"
              >
                <div>
                  <Label>Article</Label>
                  <Select
                    value={l.article_id}
                    onValueChange={(v) => modifierLigne(l.cleLigne, { article_id: v })}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {(articles ?? []).map((ar) => (
                        <SelectItem key={ar.id} value={ar.id}>
                          {ar.nom} — {formaterFCFA(ar.prix)}
                          {ar.type_prix === "kilo" ? "/kg" : ""}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                {a?.type_prix === "kilo" ? (
                  <div>
                    ...
                    <Input
                      ...
                      value={l.poids_kg ?? ""}
                      onChange={(e) =>
                        modifierLigne(l.cleLigne, {
                          poids_kg: e.target.value === "" ? null : parseFloat(e.target.value),
                        })
                      }
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        modifierLigne(l.cleLigne, {
                          quantite: e.target.value === "" ? null : parseInt(e.target.value, 10),
                        })
                      }
                      disabled={!a}
                    />
                ...
                <div className="text-right">
                  ...
                  <p className="pt-2 font-semibold text-primary">
                    {formaterFCFA(sousTotal(l))}
                  </p>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => supprimerLigne(l.cleLigne)}
                >
